...
function TimetableGrid({
  ...
  // Build slot lookup: `${periodId}-${day}` -> slot
  const slotMap = useMemo(() => {
    const map: Record<string, any> = {};
    slots.forEach((s: any) => {
      const day = s.day_of_week ?? s.dayOfWeek ?? s.day ?? "";
      const periodId =
        s.period_definition_id ?? s.periodDefinitionId ?? s.period_id ?? "";
      if (day && periodId) {
        map[`${periodId}-${day}`] = s;
      }
    });
      ...
      <Card className="border-dashed border-2">
        <CardContent className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground max-w-sm">
            Your class timetable has not been configured. Please contact your
...
export default function StudentTimetablePage() {
  ...
  const sectionId =
    student?.current_section_id ?? student?.section_id ?? student?.sectionId;

  const { data: slotsRaw, isLoading: slotsLoading } =
    useSectionTimetable(sectionId);
  const { data: periodsRaw, isLoading: periodsLoading } =
    usePeriodDefinitions();

  // Derived data
  const slots = useMemo(() => extractArray(slotsRaw), [slotsRaw]);
  ...
  const isLoading = studentLoading || slotsLoading || periodsLoading;
